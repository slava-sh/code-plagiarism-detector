...
{
    if (fabsl(dist(center, a) - r) < 1e-4 || fabsl(dist(center, b) - r) < 1e-4)
        return true;
    if ((dist(center, a) - r > -EPS && r - dist(center, b) > -EPS) || (r - dist(center, a) > -EPS && dist(center, b) - r > -EPS))
        return true;
    return false;
}
...
{
    return fabsl(line.a * a.x + line.b * a.y + line.c) < 1e-4;
}

bool equal(Point a, Point b)
{
    return (fabsl(a.x - b.x) < EPS && fabsl(a.y - b.y) < 1e-4);
}
...
bool in_otr(Point a, Point b, Point c)
{
    if (equal(a, c) || equal(b, c))
        return true;
    return (max(a.x, b.x) - c.x > -EPS && c.x - min(a.x, b.x) > -EPS && c.y - max(a.y, b.y) > -EPS && min(a.y, b.y) - c.y > -EPS);
}
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (good(polygon[i], polygon[i + 1], r))
            {
                ...
                else
                {
                    ...
                    y1 = sqrtl(r * r - x * x);
                    y2 = -sqrtl(r * r - x * x);
                    if (in_line(line, Point(x, y1)) && in_otr(polygon[i], polygon[i + 1], Point(x, y1)))
                        inter.push_back(Point(x, y1));
                    if (in_line(line, Point(x, y2)) && in_otr(polygon[i], polygon[i + 1], Point(x, y2)))
                        inter.push_back(Point(x, y2));
                }
            }
        }
