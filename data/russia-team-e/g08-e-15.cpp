...
pair<string, pair<string, string> > p1[100002];
string x[100002],k,z,l;
main(){
...
if(l!=p1[j].first){if(l>p1[j].first) {j++;i--;}else x[p[i].second]="Incorrect";continue;} else{
                                     if(x[p[i].second][0]=='0'){x[p[i].second]="Incorrect";j++;continue;}
                                     if(x[p[i].second][p1[j].second.first.size()]=='0'){x[p[i].second]="Incorrect";j++;continue;}
...
for(i=0;i<m;i++){
if(x[i][0]=='+')cout<<x[i]<<endl; else cout<<"Incorrect"<<endl;
}
