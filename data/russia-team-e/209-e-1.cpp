...
const long long mpow = 10;//100057;

vector <long long> gos[4], oper[6];
...
int main()
{
    ...
    for(int i = 0; i < n; ++i)
    {
        ...
        int k;
        cin >> c >> k;
        gos[c.length()].push_back(get_hash(c));
        for(int j = 0; j < k; ++j)
        {
            string t;
            cin >> t;
            oper[t.length()].push_back(get_hash(t));
        }
    }
    for(int i = 1; i <= 3; ++i)
    ...
    for(int i = 0; i < m; ++i)
    {
        ...
        for(leng = 1; leng <= 3; ++leng)
        {
            ...
            for(lenop = 3; lenop <= 5; ++lenop)
            {
                ...
                if(oper[lenop][left] == lenophash && norm(s, leng + lenop))
                {
                    pos = true;
                    break;
                }
