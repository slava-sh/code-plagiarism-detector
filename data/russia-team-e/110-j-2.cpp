        ...
        for (int j = 0; j < h; j++)
            for (int k = 0; k < 4; k++)
                if (Map[i][j] == 'T' && i + dx[k] >= 0 && i + dx[k] < w && j + dy[k] >= 0 && j + dy[k] < h && (Map[i + dx[k]][j + dy[k]] == 'T' || Map[i + dx[k]][j + dy[k]] == 'Z'))
                    Map[i][j] = 'Z';
    int x1 = 10000000, y1 = 10000000, x2 = 10000000, y2 = 10000000;
    for (int i = 0; i < w; i++)
        for (int j = 0; j < h; j++)
        {
            ...
            if (y2 >= sy)
            {
                ...
                for (int i = 0; i <= x2 - sx; i++)
                    for (int j = 0; j <= y2 - sy; j++)
                        D[i + 1][j + 1] = max(D[i][j + 1], D[i + 1][j]) + (Map[sx + i][sy + j] == 'T' || Map[sx + i][sy + j] == 'Z');
