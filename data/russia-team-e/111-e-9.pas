var a:array[0..111]of string;
    b:array[0..111,0..1111]of string;
    m:array[0..111]of longint;
    aa,ans,qq,k,n,i,j,w,e:longint;f:boolean;q:string;
Begin
      ...
      begin
       for w:=1 to m[j] do
        begin
         ...
         if f then
          begin
           if a[j][1]='0' then begin f:=false;continue;end;
           if b[j][w][1]='0' then begin f:=false;continue;end;
           if q[length(a[j])+length(b[j,w])+1]='0' then begin f:=false;continue; end;
           write('+',a[j],'(',b[j,w],')');
           ans:=length(q)-length(a[j])-length(b[j,w]);
           if ans=3 then begin for e:=length(a[j])+length(b[j,w])+1 to length(q) do write(q[e]); end else
           if ans=4 then 
            begin
             qq:=0;
