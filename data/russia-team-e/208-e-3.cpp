	...
	for (int i = 0; i<m; i++) {
		...
		for (int j = 0; j<11; j++) {
			...
			if (work == 0) {
				...
				if (co[temp] && s[j+1]!='0') {work++; country=temp; temp="";continue;} else {continue;}
			}
			if (work == 1) {
				if (temp.length()>5) {work=0;j-=6;temp = country;country="";continue;}
				if (op[country+"-"+temp] && s[j+1]!='0') {work++; reg=temp; temp="";continue;} else {continue;}
			}
