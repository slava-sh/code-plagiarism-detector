...
if length(s) = 5 then
 begin
  s1:=s[1]+s[2]+s[3]+'-'+s[4]+s[5];
  if (s[1]='0') then fff:=false;
 end;
if length(s) = 6 then
begin
 s1:=s[1]+s[2]+'-'+s[3]+s[4]+'-'+s[5]+s[6];
  if ((s[1]='0')) then fff:=false;
end;
if length(s) = 7 then
begin
...
Begin
   ...
   s1:='+'+a[ct].c;
   if s1[2] = '0' then begin writeln(output1,'Incorrect'); continue; end;
   f:=false;
   ...
   s1:=s1+'('+a[ct].ops[ct2]+')';
  if a[ct].ops[ct2][1] = '0' then begin writeln(output1,'Incorrect'); continue; end;
   s2:=give(s);
     if not fff then begin writeln(output1,'Incorrect'); continue; end;
