...
const int maxn = 505;
vector<pair<int,int> >len_i[1005];
char c[maxn][maxn];
char WENS[maxn][maxn];
...
int main(){
    ...
    pair<int,int>AAA[2];
    char CE[2];
    AAA[0] = pair<int,int>(result_I,result_J);
    AAA[1] = pair<int,int>(I_id,J_id);

    if(I_id > result_I)CE[0] = 'S', CE[1] = 'N'; else
    if(I_id < result_I)CE[0] = 'N', CE[1] = 'S'; else
    if(J_id > result_J)CE[0] = 'E', CE[1] = 'W'; else
    if(J_id > result_J)CE[0] = 'W', CE[1] = 'E';
    int II = 0;
    while(t > 0){
        putchar(CE[II]);
        II = !II;
        t -= c[AAA[II].first][AAA[II].second] == 'T';
    }
    return 0;
}
