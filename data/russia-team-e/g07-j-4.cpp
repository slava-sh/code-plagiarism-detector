...
main ()
 {
  ...
  for (i=1;i<=w;i++)
   for (j=1;j<=h;j++)
    { 
     if (ans[i][j] == n && abs(indi-i)+abs(indj-j) < res && a[i][j] == 'T')
      {
  ...
  if (p == 3)
   {
    ...
    while (num > 0)
     {
      if (a[resi][resj-1] == 'T') {cout<<"W"; resj--; num--; continue;}
      if (a[resi][resj+1] == 'T') {cout<<"E"; resj++; num--; continue;}
      if (a[resi-1][resj] == 'T') {cout<<"N"; resi--; num--; continue;}
      if (a[resi+1][resj] == 'T') {cout<<"S"; resi++; num--; continue;}
     }
    cout<<endl;
   }
 }
