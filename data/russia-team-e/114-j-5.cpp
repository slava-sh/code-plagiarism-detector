...
int main() 
{
	...
	while (!q.empty())
	{
		...
		for (k = 0; k < 4; k++)
		{
			...
			int nd = d2[p.x][p.y] + (t[too.x][too.y] != 'T');
			if (nd < d2[too.x][too.y] || d2[too.x][too.y] == 0)
			{
				d2[too.x][too.y] = nd;
				from2[too.x][too.y] = p;
				len[too.x][too.y] = len[p.x][p.y] +  (t[too.x][too.y] == 'T');
				if (nd == d2[p.x][p.y]) q.push_front(too);
				else q.push_back(too);
			}
		}
	}
