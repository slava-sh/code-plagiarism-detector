...
int main()
{
	...
	for (uint32_t i = 0; i < m; i++)
	{
		...
		map[from].emplace_back(road_t {to, temp, i});
		map[to].emplace_back(road_t {from, temp, i});
	}
		...
		while (!the_queue.empty())
		{
			...
			else
			{
				for (road_t r: map[cur.first])
					if (visited[cur.first].find(r.road_id) == visited[cur.first].end() and (cur.second == INT32_MAX or uint32_t(smart_abs(r.temp, cur.second)) <= d) )
					{
						visited[cur.first].insert(r.road_id);
