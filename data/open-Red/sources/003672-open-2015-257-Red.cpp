...
int main(){
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < (int)g[i].size(); ++j) {
			g[i][j].i = bin(g[g[i][j].v], edge(g[i][j].t, i, g[i][j].m ^ 1));
		}
	}
	for (int i = 0; i < n; ++i) {
	...
	for (;q; --q) {
		...
		while (l < r) {
			...
			/*for (int j = 0; j < (int)g[v].size(); ++j) {
				if (abs(g[v][j].t - g[v][i].t) <= d) {
			del(a, i);
					if (dist(g[v][j].t
				}
			}*/
			for (int j = g[v][i].l; j != -1 && g[v][i].t - g[v][j].t <= d; j = g[v][j].l) {
			...
			for (int j = g[v][i].r; j != -1 && g[v][j].t - g[v][i].t <= d; j = g[v][j].r) {
				if (dist[g[v][j].v][g[v][j].i] < infi) {
					break;
				}
				och[r] = mp(g[v][j].v, g[v][j].i);
