...
int zapr(int a, int b)
{
    ...
    while (t.size() && (t.front()!=b))
    {
     ...
     //   cerr << _t + 1 << " " << _nst << " " << _temp << " " << _ost << "\n";
        for (int i = _ost; i>=0 && _temp - temp[_t][i] <= d; i--)
        if (!pos[_t][i])
        {
            t.push(reb[_t][i]);
            nst.push(_nst + 1);
        ...
        for (int i = _ost + 1; i<reb[_t].size() &&  (temp[_t][i] - _temp <= d); i++)
        if (!pos[_t][i])
        {
            t.push(reb[_t][i]);
            nst.push(_nst + 1);
...
int main()
{
 ...
 //   freopen("output.txt", "w", stdout);
    ...
    for (int i=0; i<m; i++)
    {
        int a, b, c;
        cin >> a >> b >> c;
        if (reb[a-1].size()==0 || !(reb[a-1][reb[a-1].size()-1] == b-1 && temp[a-1][reb[a-1].size()-1] == c))
        {
    ...
    for (int i=0; i<q; i++)
    {
        ...
        for (int l=0; l<n; l++)
            for (int k=0; k<pos[l].size(); k++)
                pos[l][k] = 0;
        if (!wres.count(mp(h, j)))
        {
            wres[mp(h, j)] = zapr(h-1, j-1);
        }
        cout << wres[mp(h, j)] << "\n";
    }
