...
type
  ...
  mass = array[1..2*mm] of int64;

var
  graph:mass;
  last:array[1..mn] of longint;
  ans:array[1..mn] of int64;
  prev:mass;
  time:mass;
  used:mass;
  hot:mass;
...
begin
 ...
 // ans[u]:=0;
  ...
  begin
   time[i]:=0;
  // ans[graph[i]]:=1;
   refresh(i,root,1,2*m);
   i:=prev[i];
  end;
  i:=1;
  While (time[root^.ind] < high(longint)) and (i<=2*m) and (ans[v]>=high(longint)) do
  begin
   inc(i);
   k:=root^.ind;
   j:=last[graph[k]];
   ...
   begin
    ...
    begin
     If time[j]>time[k]+1 Then
     begin
      time[j]:=time[k]+1;
      If (ans[graph[j]]>time[j]+1) and (graph[j]=v) Then ans[graph[j]]:=time[j]+1;
