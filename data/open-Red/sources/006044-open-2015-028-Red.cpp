...
int main() {
    ...
    while (q--) {
        ...
        vector<vi> id(2, vi(m, 0));
        for (int i = 0; i < g[a].size(); ++i)
            if (x[g[a][i]] == a)
                d[1][g[a][i]] = 1, q.push(mp(-1, mp(g[a][i], 1))), id[1][g[a][i]] = 1;
            else
                d[0][g[a][i]] = 1, q.push(mp(-1, mp(g[a][i], 0))), id[0][g[a][i]] = 1;
        int res = INF;
        while (!q.empty()) {
            int num = q.top().S.F, dir = q.top().S.S;
            int dist = -q.top().F;
            q.pop();
            if (dist != d[dir][num])
                continue;
            ...
            for (int i = 0; i < g[a].size(); ++i)
                if (abs(1ll * t[num] - t[g[a][i]]) <= D) {
                    ...
                    if (d[ndir][g[a][i]] > d[dir][num] + 1) {
                        ...
                        q.push(mp(-d[ndir][g[a][i]], mp(g[a][i], ndir)));
                    }
                }
        }
