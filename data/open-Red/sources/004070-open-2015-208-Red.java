...
public class Red {
	...
	class Edge {
		...
		public Edge(int u, long w, int ind, int obrInd) {
			this.u = u;
			this.w = w;
			this.ind = ind;
			this.obrInd = obrInd;
	...
	int dijkstra(int st, int fn) {
		...
		TreeSet<Triple> min = new TreeSet<Triple>();
		for (int i = 0; i < n; i++) {
		...
		while (!min.isEmpty()) {
			...
			for (Edge e : eds[cur.v]) {
				...
				if ((Math.abs(e.w - eds[cur.v].get(cur.lastEdge).w) <= d || cur.v == st)
						&& dist[e.u].get(e.obrInd) > dist[cur.v]
								.get(cur.lastEdge) + 1) {
					min.remove(new Triple(e.u, e.obrInd, dist[e.u]
							.get(e.obrInd)));
					dist[e.u].set(e.obrInd, dist[cur.v].get(cur.lastEdge) + 1);
					min.add(new Triple(e.u, e.obrInd, dist[e.u].get(e.obrInd)));
				}
			}
	...
	void solve() throws IOException {
		...
		for (int i = 0; i < m; i++) {
			...
			int b = nextInt() - 1;
			int w = nextInt();
			eds[a].add(new Edge(b, w, eds[a].size(), eds[b].size()));
			eds[b].add(new Edge(a, w, eds[b].size(), eds[a].size() - 1));
		}
		// for(int i = 0; i < n; i++) {
		// for(Edge e : eds[i]) {
