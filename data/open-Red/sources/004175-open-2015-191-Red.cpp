...
vector<map<llg, int> > temper;
vector<map<llg, int> > changed;
queue<int> que;
...
int solve(int begin, int end, int d)
{
	...
	while (!que.empty())
	{
		...
		if (changed[vertex].size() == 0)
			continue;
		forit(it, changed[vertex])
		{
			if (vertex == end)
				tmin = min(tmin, it->second);
			find_it = upper_bound(g[vertex].begin(), g[vertex].end(), m_p(0, it->first - d - 1), comp);
			...
			while (find_it != g[vertex].end())
			{
				...
				{
					if (changed[to].find(td) == changed[to].end() || changed[to][td] > it->second + 1)
					{
