...
int main()
{
    ...
    go.resize(f.size(), vector<int>(f.size()));

    size_t fsize = f.size();

    for (size_t i = 0; i < fsize; i++)
        for (size_t j = i; j < fsize; j++)
            if (N % (f[i] * f[j]) == 0) go[i][j] = go[j][i] = lower_bound(f.begin(), f.end(), f[i] * f[j]) - f.begin();
            else go[i][j] = go[j][i] = -1;
    ...
    for (int k = 0; k < K; k++)
        for (size_t i = 0; i < fsize; i++) {
            ...
            for (size_t j = 0; j < fsize; j++)
                if (go[i][j] != -1) {
                    minimize(dp[k + 1][go[i][j]], dp[k][i] * ((f[j] + 1) / 2));
                }
        }
