...
int64_t ans(int64_t n, int k)
{
    ...
    int am = 0;
    am = 0;
    for (auto i = x; i && am < 1; i--)
        if (n % i == 0)
        {
            debuh = m[n][k] = min(m[n][k], min(ans(n / (i), k - 1) * (((i) + 1) / 2), ans((i), k - 1) * ((n / (i) + 1) / 2)));
            debuh = m[n][k] = min(m[n][k], min(ans(n / (n / i), k - 1) * (((n / i) + 1) / 2), ans((n / i), k - 1) * ((n / (n / i) + 1) / 2)));
            am++;
        }
    if (n % 2 == 0)
...
//    {
//        n = (((((int64_t(rand()) << 16) + int64_t(rand())) << 16) + int64_t(rand())) << 16) + int64_t(rand());
//        n %= 1000000000 * 1000000ll;
//        n = n + 1000000000 * 1000000ll;
//        n %= 1000000000 * 1000000ll;
//        k = rand() % 10 + 1;
    cin >> n >> k;
//    divc(n);
