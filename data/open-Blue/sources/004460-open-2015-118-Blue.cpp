...
const int AMN = 2;
const ll MODS[AMN] = {1000000003, 1000000007};
const ll PS[AMN] = {5, 7};
ll KS[AMN][N];
ll HS[AMN][N];

//bool eq[1007][1007][51];

inline bool check(int i, int j, int len)
{
	return i == n || (i + len <= n && subs[i][len] == subs[j][len]);
}
...
bool check(int i)
{
	return i == n || (check(i, i1, l1) && check(i + l1)) || (check(i, i2, l2) && check(i + l2));
}
...
void run()
{
	...
	{
		int ist = 0;
		while (check(ist, 0, l1))
			ist += l1;
		for (l2 = 1; l2 <= l && (l1 + l2 <= n); l2++)
		{
			i1 = 0;
			i2 = ist;
			if ((check(n - l1, 0, l1) || check(n - l2, ist, l2)) && check(0))
			{
				printf("%d\n", 2);
				cout << s.substr(0, l1) << endl;
