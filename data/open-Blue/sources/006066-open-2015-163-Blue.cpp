...
inline bool cmp(bool is_a){
	if(is_a){
		...
		return true;
#endif
		return bhash == (MOD2 + shash[pos + bsize] - shash[pos] * pow5[bsize]) % MOD;
	}
}
...
bool check2(){
	...
	pow5.resize(ss + 1, 1);
	for(int i = 0; i < ss; i++){
		pow5[i+1] = (pow5[i] * BASE) % MOD;
		shash[i+1] = (shash[i] * BASE + s[i] + 1 - 'a') % MOD;
	}
	for(asize = 1; asize <= l; asize++){
		...
		for(bsize = 1; bsize * 2 + bb <= ss && bsize <= l; bsize++){
			pos = bb + bsize;
			bhash = (MOD2 + shash[bb + bsize] - shash[bb] * pow5[bsize]) % MOD;
			bool xadd = true;
			while(pos < ss && xadd){
				xadd = false;
				if(cmp(true)){
