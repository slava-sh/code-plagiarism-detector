...
bool check2(){
	...
	ull ahash, bhash;
	for(int asize = 1; asize <= l; asize++){
		ahash = shash[asize];
		int pos = asize;
		while(pos + asize <= ss && ahash == (MOD2 + shash[pos + asize] - shash[pos] * pow5[asize]) % MOD){
			pos += asize;
		}
		if(pos + l >= s.size()){
			cout << "2\n" << s.substr(0, asize) << "\n" << s.substr(pos, l);
		...
		int bb = pos;
		for(int bsize = 1; bsize * 2 + bb <= ss && bsize <= l; bsize++){
			pos = bb + bsize;
			...
			while(pos < ss && xadd){
				xadd = false;
				if(pos + asize <= ss){
					if(ahash == (MOD2 + shash[pos + asize] - shash[pos] * pow5[asize]) % MOD){
						pos += asize;
						xadd = true;
					}
				}
				if(pos + bsize <= ss){
					if(bhash == (MOD2 + shash[pos + bsize] - shash[pos] * pow5[bsize]) % MOD){
						pos += bsize;
						xadd = true;
					}
				}
