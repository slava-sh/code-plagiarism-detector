...
int main() {
	...
	for (int i = 0; i <= n; ++i) {
		for (int j = i; j <= n; ++j) {
			...
			set <string> cur2 = calc(str.substr(i, j - i));
			set <string> cur3 = calc(str.substr(j));
			//cout << str.substr(0, i) << ";" << str.substr(i, j - i) << ";" << str.substr(j) << endl;
			cur.insert(cur2.begin(), cur2.end());
			cur.insert(cur3.begin(), cur3.end());
