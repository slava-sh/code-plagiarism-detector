	...
	if (z.size() == 3) {
		...
		for (ll i = 0; i < n; ++i) {
			...
			for (ll j = i; j < min<ll>(i+l, n); ++j) {
				cs.push_back(s[j]);
				if (sss.find(cs) == sss.end()) {
					sss.insert(cs);
					ss.push_back(pll(i, j));
				}
			}
		}
		for (ll ai = 0; ai < ss.size(); ++ai) {
			if (ss[ai].first != 0) {
				...
				string bs = s.substr(0, ss[ai].first);
				ll b = hs.hash(0, ss[ai].first-1);
				...
				can[0] = true;
				for (ll i = 1; i <= n; ++i) {
					if (i-al>=0&&can[i-al]&&s.substr(i-al, al-1)==as) {
						can[i] = true;
					}
					if (i-bl>=0&&can[i-bl]&&s.substr(i-bl, bl)==bs) {
						can[i] = true;
					}
				}
				if (can[n]) {
