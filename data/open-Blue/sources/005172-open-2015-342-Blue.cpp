...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        string t;
        long long hs1 = 0;
        for (int j = i; j >= max(i - min(l, 50) + 1, 0); j--) {
            hs *= P;
            hs += s[j];
            hs %= M;
            hs1 *= P1;
   ...
   // cout << "opa" << endl;
    ...
    for (int j = m - 1; j >= 1; j--) {
        for (int i = j - 1; i >= 0; i--) {
            if (mapi[v[i].first][0] != s[0] && mapi[v[j].first][0] != s[0]) {
                continue;
            }
            if (mapi[v[i].first].back() != s.back() && mapi[v[j].first].back() != s.back()) {
                continue;
            }
            if (gethash(0, v[i].second - 1) != v[i].first.first || gethash1(0, v[i].second - 1) != v[i].first.second) {
                if (gethash(0, v[j].second - 1) != v[j].first.first || gethash1(0, v[j].second - 1) != v[j].first.second) {
                    continue;
                }
            }
            if (gethash(n - v[i].second, n - 1) != v[i].first.first || gethash1(n - v[i].second, n - 1) != v[i].first.second) {
                if (gethash(n - v[j].second, n - 1) != v[j].first.first || gethash1(n - v[j].second, n - 1) != v[j].first.second) {
                    continue;
                }
            }
            t++;
