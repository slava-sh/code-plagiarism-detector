...
pair<string, string> get_period2(const string& s)
{
	int n = s.length();
	int bound = l;
	...
	for (int x = 1; x <= bound; x++)
	{
		...
		for (int i = 0; i < n; i++)
		{
			h.first += ((s[i] - 'a' + 1) * p_pow1[i - last]) % mod1;
			h.first %= mod1;
			h.second += ((s[i] - 'a' + 1) * p_pow2[i - last]) % mod2;
			h.second %= mod2;
		...
		return make_pair(s.substr(0, x), s2);
	}
