    ...
    for (int i = 1; i <= n && i <= l; ++i) {
        ...
        for (int j = 1; j + j <= R-L+1 && j <= l; ++j) {
            if (get_hash(L, L+j-1) == get_hash(R-j+1, R)) {
                int prevl = L, prevr = R;
                ll curhash2 = get_hash(L, L+j-1);

                while (L <= R) {
                    if (L+j-1 <= R && get_hash(L, L+j-1) == curhash2) L += j;
                    else if (L+i-1 <= R && get_hash(L, L+i-1) == curhash) L += i;
                    else if (R-j+1 >= L && get_hash(R-j+1, R) == curhash2) R -= j;
                    else if (R-i+1 >= L && get_hash(R-i+1, R) == curhash) R -= i;
                    else break;
                }
                if (L > R) {
                    printf("2\n");
