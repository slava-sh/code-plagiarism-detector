   ...
   // cout << "opa" << endl;
    ...
    for (int i = 0; i < m; i++) {
        for (int j = i + 1; j < m; j++) {
            ...
            for (int k = 0; k < n; k++) {
                if (k + 1 >= v[i].second) {
                    if (gethash(k - v[i].second + 1, k) == v[i].first.first && gethash1(k - v[i].second + 1, k) == v[i].first.second) {
                        d[k] = 1;
                        continue;
                    }
                }
                if (k + 1 >= v[j].second) {
                    if (gethash(k - v[j].second + 1, k) == v[j].first.first && gethash1(k - v[j].second + 1, k) == v[j].first.second) {
                        d[k] = 1;
                        continue;
                    }
                }
