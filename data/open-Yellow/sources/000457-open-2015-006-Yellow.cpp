...
int main() {
	...
	for (int i = 0; i < q; i++) {
		...
		pair <pair <int, int>, int> f = (*(--s.upper_bound(make_pair(make_pair(x, (int)2e9), -2e9)))), p;
		if (f.second == y) {
			printf("%d\n", (int)s.size());
			continue;
		}
		...
		set <pair <pair <int, int>, int> >::iterator it = s.lower_bound(make_pair(make_pair(x, x), -2e9));
		...
		it = s.lower_bound(make_pair(make_pair(x, x), -2e9));
