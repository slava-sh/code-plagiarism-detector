...
int main()
{
	...
	vector<uint64_t> dp(K + 1, UINT64_MAX);
	dp[K] = 0; 
	...
	for (uint32_t x = 1; x <= N; x++)
	{
		...
		vector<uint64_t> new_dp(K + 1);
		
		for (uint32_t i = 0; i <= K; i++)
		{
			new_dp[i] = (i != K) ? (dp[i + 1]) : (UINT64_MAX);
			if (i == K - 1)
				new_dp[i] = min(new_dp[i], dp[0] == UINT64_MAX ? UINT64_MAX : (dp[0] + C));
			if (x != 1)
				new_dp[i] = min(new_dp[i], dp[i] == UINT64_MAX ? UINT64_MAX : (dp[i] + 1LL * D * (a - prev_b)));
			if (new_dp[i] != UINT64_MAX)
				new_dp[i] += 1LL * D * (b - a);
		}
		prev_b = b;
		dp = std::move(new_dp);
