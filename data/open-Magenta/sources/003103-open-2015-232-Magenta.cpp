...
int m[12] = {10, 30, 76, 169, 339, 624, 1074, 1744, 2697, 3996, 5706, 6000};

int m1[15] = {10, 27, 61, 123, 224, 378, 600, 904, 1302, 1805, 2420, 3152, 4002,
4968, 6000};
...
int main(){
	...
	for(int i = 1; ch == 'N'; ++i){
		...
		for(int q = 1; q <= m[i] && ch == 'N'; ++q){
			while(y * (x + 1) <= m[i] && ch == 'N') command('U');
			while(x >= 0 && f[y][x - 1] != step - 1 && ch == 'N') command('D');
			if(ch == 'N')command('L');
		}
		for(int i = 0; i < m[i] && ch == 'N'; ++i)
			command('R');
		for(int i = 0; i < m[i] && ch == 'N'; ++i)
			command('U');
		/*for(int q = 1; q <= m[i] && ch == 'N'; ++q){
