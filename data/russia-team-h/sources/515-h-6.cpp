...
int deij(int s)
{
	...
	for(z = 0; z < X; z++)
	{
		...
		for(i = 0; i < X; i++)
		{
			if(new_table[k][i] != 0)
			{
				if(d[i] >  d[k] + new_table[k][i])
				{
					pr[i] = k;
				}
				d[i] = min(d[i], d[k] + new_table[k][i]); 
			}
		}
...
int main()
{
	...
	for(i = 0; i < n; i++)
	{
		if(color[i] == 0)
		{
			...
			if(new_table[i + 2][0] == INF)
			{
				new_table[i + 2][0] = 0;
				new_table[0][i + 2] = 0;
			}
			if(new_table[i + 2][1] == INF)
			{
				new_table[i + 2][1] = 0;
				new_table[1][i + 2] = 0;
			}
	...
	for(i = 0; i < n; i++)
	{
		if(table[g[g.size() - 2] - 2][i] == d[1] - d[g[g.size() - 2]])
		{
			y = i;
		}
	}
