...
begin
	...
	a := cross_product( 0, 0, x1 - x3, y1 - y3, vx, vy );
	b := cross_product( 0, 0, ux, uy, vx, vy );
...
var	minX, maxX, m, i, j, k, x0, y0, x1, y1, x2, y2, len, p1, p2, magic: longInt;
	segments, seg: tSegArr;
	o, w, firstDiff: array[ 1..100000 ] of longInt;

begin
	randomize();
	magic := random( 1000000000 );
...
//				writeln( seg[j].id, ' ', seg[j].x1, ' up ', p1, ' -> ', p2 );
...
//				writeln( seg[j].id, ' ', seg[j].x1, ' down ', p1, ' -> ', p2 );
