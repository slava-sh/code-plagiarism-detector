...
int main()
{
	...
	{
		goodpos0.push_back(coord[i] + 0.000000001);
		goodpos0.push_back((coord[i + 1] + coord[i]) * 0.5 + 0.000000000001);
	}
	goodpos0.push_back(coord.back() - 0.000000001);
	sort(goodpos0.begin(), goodpos0.end());
	...
	for(int i = 0; i < coord.size(); i++)
		goodpos.push_back(coord[i] + 0.000001);
	...
	for(int i = 0; i < N; i++)
	{
		for(int j = 0; j < N; j++)
		{
			bool b4 = y(goodpos[0], i) > y(goodpos[0], j);
			bool b1 = y(goodpos[0], i) < y(goodpos[0], j);
			if(b4 == 0 && b1 == 0)
			for(int k = 0; k < 100; k++)
			{
				double curX = (double)k * 0.01 * (B - A) + A;
				b4 = y(curX, i) > y(curX, j);
				b1 = y(curX, i) < y(curX, j);
				if(b4 || b1)
					break;
			}
			...
			for(int k = 0; k < c; k++)
			{
				bool b2 = y(goodpos[k], i) < y(goodpos[k], j);
				bool b3 = y(goodpos[k], i) > y(goodpos[k], j);
