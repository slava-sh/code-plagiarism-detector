...
public:
	bool operator()(pair<int, int> a, pair<int, int> b){
		...
		if(tmp != M.end()){
            if(a.first > b.first) swap(a, b), f = 0;
            ld ay = line[a.first][a.second].get_y(X);
            ld by = line[b.first][b.second].get_y(X);
            if((ay > by + eps && tmp->second == 1) || (by > ay + eps && tmp->second == -1)){
                cout << "No\n";
                cout << a.first + 1 << ' ' << b.first + 1;
                exit(0);
            }else{
                if(tmp->second == 1) return f;
                return !f;
            }
		}else{
            if(a.first > b.first) swap(a, b), f = 0;
            ...
            ld ay = line[a.first][a.second].get_y(X);
            ld by = line[b.first][b.second].get_y(X);
            if(ay > by + eps) ans = M[tform(a.first, b.first)] = -1;
            else if(by > ay + eps) ans = M[tform(a.first, b.first)] = 1;
            if(ans == 1) return f;
            if(ans == -1) return !f;
            return 1;
		}
