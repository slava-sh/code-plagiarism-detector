...
/*void tmp_check_intersection(const Curve &a, const Curve &b) {
    bool cmp_begins = cmp_begin_curve(a, b);

    for (auto &p: a.points) {
        int x = p.x;
        int l = 0, r = b.points.size();
        //!!!
    }
}*/
...
void check_intersection(const Curve &a, const Curve &b) {
    ...
    bool a_is_less = a.points[0].y < b.points[0].y;

    for (int j = 1; j < (int) a.points.size() - 1; j++) {
