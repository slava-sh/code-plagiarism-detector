...
{
	int minx = min(min(s1.p1.x, s2.p1.x), min(s1.p2.x, s2.p2.x));
	int maxx = max(max(s1.p1.x, s2.p1.x), max(s1.p2.x, s2.p2.x));
	int miny = min(min(s1.p1.y, s2.p1.y), min(s1.p2.y, s2.p2.y));
	int maxy = max(max(s1.p1.y, s2.p1.y), max(s1.p2.y, s2.p2.y));
...
bool check(Line l1, Line l2)
{
	if(l2.b * l1.a - l2.a * l1.b == 0)// параллельны
		return false;
	int y = (l2.a * l1.c - l1.a * l2.c) / (l2.b * l1.a - l2.a * l1.b);
	int x = (l1.b * l2.c - l1.c * l2.b) / (l2.b * l1.a - l2.a * l1.b);
