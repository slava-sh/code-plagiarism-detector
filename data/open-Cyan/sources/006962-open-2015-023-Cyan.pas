...
begin
	...
	for i := 1 to n do
		...
		begin
			...
			begin
				for k := max( x[i][u - 1], x[j][v - 1] ) to min( x[i][u], x[j][v] ) do
				begin
					if ( compareSegmentsAtX( x[i][u - 1], y[i][u - 1], x[i][u], y[i][u], x[j][v - 1], y[j][v - 1], x[j][v], y[j][v], k ) = -1 ) then
						p := true;
