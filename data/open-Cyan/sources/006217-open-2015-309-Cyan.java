...
public class Main {
    ...
    private static class Curve implements Comparable<Curve> {
        ...
        public int compareTo(Curve o) {
            /*for (int i = 0; i < Math.min(X.length, o.X.length); ++i) {
            ...
            for (int x : X) Q.add(x);
            for (int x : o.X) Q.add(x);
            while (!Q.isEmpty()) {
                int trial = Q.remove();
                double pos = getPosition(trial);
                double otherPos = getPosition(trial);
                if (pos > otherPos + EPS) return 1;
                else if (pos + EPS < otherPos) return 1;
            }
            return 0;
        }
