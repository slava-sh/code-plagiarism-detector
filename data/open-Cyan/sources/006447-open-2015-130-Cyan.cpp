...
int main(){
	...
	for (int i = 0; i < n; ++i)
        for (int j = i + 1; j < n; ++j){
            ...
            b = 1;
            i1 = i2 = 0;
            merge(vc[i].begin(), vc[i].end(), vc[j].begin(), vc[j].end(), m1);
            for (int k = 0; k < vc[i].size() + vc[j].size(); ++k){
                if (m1[k] > vc[i][a])
                    ++a;
                if (m1[k] > vc[j][b])
                    ++b;
                d1 = gt(vc[i][a - 1], vc1[i][a - 1], vc[i][a], vc1[i][a], m1[k]);
                d2 = gt(vc[j][b - 1], vc1[j][b - 1], vc[j][b], vc1[j][b], m1[k]);
                if (d1 - 1e-9 > d2)
                    i1 = 1;
                if (d2 - 1e-9 > d1)
                    i2 = 1;
