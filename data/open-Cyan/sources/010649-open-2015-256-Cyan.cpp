...
//			cout << nextY[j] << " ";
	...
	for (int i = 0; i < n; ++i) {
		minPref[i].resize(m);
		maxPref[i].resize(m);
		maxPref[i][0] = minPref[i][0] = YatX[0][i];
		for (int j = 1; j < m; ++j) {
			maxPref[i][j] = max(maxPref[i][j - 1], YatX[j][i]);
			minPref[i][j] = min(minPref[i][j - 1], YatX[j][i]);
		}
	}
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < n; ++j) {
			for (int l = 3; l < m; ++l) {
				//cout << minPref[i][l - 1] << " ";
				//cout << maxPref[j][l - 1] << "; " << YatX[l][i] << " " << YatX[l][j] << endl;
				if (minPref[i][l - 1] < maxPref[j][l - 1] && YatX[l][i] > YatX[l][j]) {
						cout << "No" << endl;
						cout << i + 1 << " " << j + 1 << endl;
						return 0;	
				}
