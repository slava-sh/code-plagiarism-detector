...
long double get_height(const Point &p, const Point &q, int x) {
    long double a = p.y - q.y;
    long double b = q.x - p.x;
    long double c = -(a * p.x + b * p.y);
    return -(c + a * x) / b;
}
...
void tmp_check_intersection(const Curve &a, const Curve &b) {
    ...
    for (int j = 0; j < a.points.size(); j++) {
        ...
        for (int i = 0; i < (int) b.points.size() - 1; i++) {
            long double y = get_height(b.points[i], b.points[i + 1], p.x);
            if (y > max(b.points[i].y, b.points[i + 1].y) ||
                y < min(b.points[i].y, b.points[i + 1].y)) continue;
            if (y < p.y) is_up = 1;
            if (y > p.y) is_down = 1;
        }
    }
...
void solve(int l, int r) {
    for (int i = 0; i < N; i++) {
        for (int j = i + 1; j < N; j++) {
            check_intersection(curves[i], curves[j]);
        }
    }
