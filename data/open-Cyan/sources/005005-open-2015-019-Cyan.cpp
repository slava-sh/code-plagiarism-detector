...
bool f (long long i, long long j) {
	...
	while (uk1 <= rz[i] && uk2 <= rz[j]) {
		...
		if (a[i][uk1].x < a[j][uk2].x) {
			if (seg(a[i][uk1], point(a[i][uk1].x, -1000000000000000000)) * seg(a[j][uk2 - 1], a[j][uk2])) ans1 = 1;
			else ans1 = -1;
			...
			if (seg(a[j][uk2], point(a[j][uk2].x, -1000000000000000000)) * seg(a[i][uk1 - 1], a[i][uk1])) ans1 = -1;
			else ans1 = 1;
