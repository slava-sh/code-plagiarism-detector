...
rn_t operator+(const rn_t &a, const rn_t &b) {
	return rn_t(a.p*b.q+a.q*b.p, a.q*b.q).norm();
}
...
inline
rn_t operator*(const rn_t &a, const rn_t &b) {
	return rn_t(a.p*b.p, a.q*b.q).norm();
}

inline
rn_t operator/(const rn_t &a, const rn_t &b) {
	return rn_t(a.p*b.q, a.q*b.p).norm();
}
		...
		for (ll j = i+1; j < n; ++j) {
			...
			for (ll k = 0; k < real[i].size(); ++k) {
				...
				if (!undef && real[i][k] != real[j][k]) {
					if (((fisover ^ (real[i][k] < real[j][k])) & 1) == 0) {
						cout << "No" << endl;
						cout << i+1 << " " << j+1 << endl;
						return 0;
					}					
