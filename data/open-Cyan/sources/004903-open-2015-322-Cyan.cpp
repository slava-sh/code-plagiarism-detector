...
int new_point(point p, int b, double k){
    return p.y + (b - p.x)*k;
}
                ...
                for (int b = l; b < mount[j].size(); b++){
                    ...
                    if (!isintersected(mount[i][a], mount[j][b]))
                        ...
                        if (p.x > mount[i][a].p1.x && p.x < mount[i][a].p2.x &&
                            ...
                            cout << "No" << endl;
                            cout << min(i + 1, j + 1) << " " << max(j + 1, i + 1) << endl;
                            return 0;
                        }
                        if ((p.x -mount[i][a].p2.x) < eps && a != mount[i].size() - 1){
                            if (new_point(mount[j][b].p2, mount[j][b].p2.x, mount[j][b].k) <
                                new_point(mount[i][a].p2, mount[j][b].p2.x, mount[i][a].k) &&
                                new_point(mount[j][b].p2, mount[j][b].p2.x, mount[j][b].k) >
                                new_point(mount[i][a + 1].p2, mount[j][b].p2.x, mount[i][a + 1].k)){
                                cout << "No" << endl;
                                cout << min(i + 1, j + 1) << " " << max(j + 1, i + 1) << endl;
                                return 0;
                            }else
