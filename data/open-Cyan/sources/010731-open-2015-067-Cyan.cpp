...
int main() {
    ...
    for (int i = 0; i < n; ++i) {        
        if (i < n - 1) {            
            ...
            if (v[i].ff != v[i + 1].ff) {
                ...
                have.insert(((LL) v[i].ss << 32) | v[i + 1].ss);
            }
        }
        if (i > 0) {
            ...
            if (v[i].ff != v[i - 1].ff) {
                ...
                have.insert(((LL) v[i - 1].ss << 32) | v[i].ss);
            }
        }
    }
    ...
    for (int i = 0; i < n; ++i) {        
        if (i < n - 1) {            
            ...
            if (v2[i].ff != v2[i + 1].ff) {
                ...
                have.insert(((LL) v2[i].ss << 32) | v2[i + 1].ss);
            }
        }
        if (i > 0) {
            ...
            if (v2[i].ff != v2[i - 1].ff) {
                ...
                have.insert(((LL) v2[i - 1].ss << 32) | v2[i].ss);
            }
        }
    }
    ...
    for (int i = 0; i < szof(scanline); ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            ...
            //if (j != -1) {
                if (checkup(inp[numl][numc], inp[j][wherenow[j]], inp[j][wherenow[j] + 1])) {
                    //graph[j].puba(numl);
                    have.insert(((LL) j << 32) | numl);
                    if (have.count(((LL) numl << 32) | j)) {
                        printf("No\n%d %d\n", numl + 1, j + 1);
                        return 0;
                    }
                }
                if (checkdown(inp[numl][numc], inp[j][wherenow[j]], inp[j][wherenow[j] + 1])) {
                    //graph[numl].puba(j);
                    have.insert(((LL) numl << 32) | j);
                    if (have.count(((LL) j << 32) | numl)) {
                        printf("No\n%d %d\n", numl + 1, j + 1);
                        return 0;
                    }
                }
