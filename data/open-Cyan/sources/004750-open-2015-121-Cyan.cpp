...
int main()
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            if (i != now_ind)
            {
                long double x1 = v[i].points[tmp[i]].first;
                long double x2 = v[i].points[tmp[i] - 1].first;
                long double y1 = v[i].points[tmp[i]].second;
                long double y2 = v[i].points[tmp[i] - 1].second;
                long double y = (y2 * now_x - y1 * now_x + y1 * x2 - y2 * x1) / (x2 - x1);
                ans[i] = y;
            }
        }
        //cout << "D" << endl;
