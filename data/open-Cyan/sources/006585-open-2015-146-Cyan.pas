...
var
 ...
 begin
  ...
  If under=0 then
  begin
   If (ta1*(v2[j+1].x-v2[j].x)>tb1*(v1[i+1].x-v1[i].x)) Then under:=1;
   If (ta1*(v2[j+1].x-v2[j].x)<tb1*(v1[i+1].x-v1[i].x)) Then under:=2;
  end;
  GetIm:=false;
  If under<> 0 then
  begin
   If (under = 1) and (ta2*(v2[j+1].x-v2[j].x)<tb2*(v1[i+1].x-v1[i].x)) Then
   begin
    GetIm:=true;
   end;
   If (under = 2) and (ta2*(v2[j+1].x-v2[j].x)>tb2*(v1[i+1].x-v1[i].x)) Then
   begin
    GetIm:=true;
   end;
  end;
...
begin
 ...
 begin
  ...
  nv1:=1;
  nv2:=1;
  While (nv1<l1+1) and (nv2<l2+1) do
  begin
