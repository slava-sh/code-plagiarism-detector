...
bool cmp(pair<int, vector<point> > a, pair<int, vector<point> > b)
{
	...
	bool f2 = false;
	int i1 = 0, i2 = 0;
	if (a.second[i1].y > b.second[i2].y)
		f1 = true;
	if (a.second[i1].y < b.second[i2].y)
		f2 = true;
	while (i1 != (int)a.second.size() - 1 && i2 != (int)b.second.size() - 1)
	{
		long double c1 = 100000000000, c2 = 100000000000;
		if (i1 + 1 < (int)a.second.size())
		{
			c1 = a.second[i1 + 1].x;
		}
		if (i2 + 1 < (int)b.second.size())
		{
			c2 = b.second[i2 + 1].x;
		}
		if (c1 < c2)
		{
			++i1;
			long double ans = getval(b.second[i2], b.second[i2 + 1], a.second[i1].x);
			if (ans > a.second[i1].y + eps)
				f2 = true;
			if (ans < a.second[i1].y - eps)
				f1 = true;
		}
		...
		{
			++i2;
			long double ans = getval(a.second[i1], a.second[i1 + 1], b.second[i2].x);
			if (ans > b.second[i2].y + eps)
				f1 = true;
			if (ans < b.second[i2].y - eps)
				f2 = true;
		}
		...
		{
			cout << "No" << endl;
			cout << min(a.first, b.first) + 1 << ' ' << max(a.first, b.first) + 1 << endl;
			exit(0);
		}
	}
	return (f2);
