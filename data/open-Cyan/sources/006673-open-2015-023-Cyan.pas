...
begin
	...
	for i := 1 to n do
		...
		begin
			...
			begin
				if ( intersects( x[i][u - 1], y[u][i - 1], x[i][u], y[i][u], x[j][v - 1], y[j][v - 1], x[j][v], y[j][v] ) ) then
				begin
					writeln( 'No' );
					writeln( i, ' ', j );
				...
				if ( compareSegments( x[i][u - 1], y[u][i - 1], x[i][u], y[i][u], x[j][v - 1], y[j][v - 1], x[j][v], y[j][v] ) = -1 ) then
					p := true;
