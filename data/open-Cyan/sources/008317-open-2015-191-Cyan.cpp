...
bool intersec(int s1, int s2)
{
	...
	int l = 0, l_n = v1.size(), r = 0, r_n = v2.size();

	pair<int, int> vec1, vec2;

	while (l != l_n && r != r_n)
	{
		while (r != r_n && v1[l].first >= v2[r].first)
		{
			if (l != 0 && v2[r].first != v1[l - 1].first)
			{
				vec1 = v2[r] - v1[l - 1];
				vec2 = v1[l] - v1[l - 1];
				if ((vec1 ^ vec2) < 0)
		...
		if (r != r_n && r - 1 >= 0 && v1[l].first >= v2[r - 1].first && v1[l].first <= v2[r].first)
		{
			if (v1[l].second < v2[r - 1].second)
				return true;
		}
		l++;
	}
	...
	while (l != l_n && r != r_n)
	{
		while (l != l_n && v2[r].first >= v1[l].first)
		{
			if (r != 0 && v1[l].first != v2[r - 1].first)
			{
				vec1 = v1[l] - v2[r - 1];
				vec2 = v2[r] - v2[r - 1];
				if ((vec1 ^ vec2) > 0)
		...
		if (l != l_n && l - 1 >= 0 && v2[r].first >= v1[l - 1].first && v2[r].first <= v1[l].first)
		{
			if (v1[l - 1].second < v2[r].second)
				return true;
		}
		r++;
	}
