...
struct tree_t {
	...
	ll ineqcnt(ll v, ll tl, ll tr, ll l, ll r, ll lv, ll rv) {
		...
		if (tl >= l && tr <= r) {
			ll L1 = lower_bound(t[v].begin(), t[v].end(), lv)-t[v].begin();
			ll R1 = lower_bound(t[v].begin(), t[v].end(), rv)-t[v].begin();
			ll L2 = lower_bound(t[v].begin(), t[v].end(), rv)-t[v].begin();
			ll R2 = lower_bound(t[v].begin(), t[v].end(), lv)-t[v].begin();
			return max<ll>(0, R1-L1+1)-max<ll>(0, R2-L2+1);
		}
		ll tm = (tl+tr)>>1;
	...
	for (ll i = 0; i < n; ++i) {
		for (ll j = i+2; j < n; ++j) {
			if (a[j]-a[i]<0) {
				...
				//rin[i][j] -= t.ineqcnt(1, 0, n-1, i+1, j-1, a[j], a[i]);
			}
		}
	}
	for (ll j = 0; j < n; ++j) {				
