...
int main()
{
    ...
    vector<vector<int> > antiinv_b(n, vector<int>(n));
    vector<vector<int> > antiinv_f(n, vector<int>(n));
    for (int i = 0; i < n; i++)
    {
        inv_b[i][i] = 0;
        for (int j = i - 1; j >= 0; j--)
        ...
        for (int j = i + 1; j < n; j++)
        {
            antiinv_f[i][j] = antiinv_f[i][j - 1];
            if (a[j] > a[i])
                antiinv_f[i][j] += 1;
        }
    }
    int sk = bubble_sort(a);
    int k = sk;
    int min_i = -1, min_j = -1;
    cerr << k << endl;
    for (int i = 0; i < n; i++)
    {
        ...
        {
            int nk = k;
            nk -= inv_b[j][i + 1] + inv_f[i][j - 1];
            nk += antiinv_f[i][j - 1] + antiinv_b[j][i + 1];
            if (a[j] > a[i])
                nk += 1;
            else
                nk -= 1;
