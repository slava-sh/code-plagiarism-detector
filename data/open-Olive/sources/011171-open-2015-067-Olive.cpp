...
/*
const size_t MAXMEM = 200 * 1024 * 1024;
char MEM[MAXMEM];
size_t NOWMEM = 0;
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        if (min(inp[i], i + 1) - segtree_sum(roots[i + 1], 0, bdv - 1, 0, inp[i]) <= ans.ff) {
            continue;
        }   
        for (int j = i + ans.ff + 1; j < n; ++j) {
            if (inp[i] - inp[j] - 1 <= ans.ff) {
                continue;
            }   
            int num = segtree_sum(roots[j], 0, bdv - 1, inp[j], inp[i] - 1) - segtree_sum(roots[i], 0, bdv - 1, inp[j], inp[i] - 1);
            
            ans = max(ans, mapa(num, mapa(i, j)));
        }
    }   
    
    //cout << segtree_sum(roots[2], 0, bdv - 1, 1, 2) << endl;
    //cout << segtree_sum(roots[4], 0, bdv - 1, 1, 2) << endl;
    //cout << ans.ff << endl;
    if (ans.ff == 0) {
        printf("-1 -1\n");
    } else {
